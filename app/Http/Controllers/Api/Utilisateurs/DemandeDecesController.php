...

{
...

    {
        ...
        try {
           ...
           // --- préparation des URLs de retour (deep link mobile) ---
...
// Optionnel : fallback web (recommandé si CinetPay refuse les schemes non-http(s))

// cette page doit rediriger vers le deep link (window.location = 'plateauapps://...') si possible
...
$cinetpayApiKey = env('CINETPAY_APIKEY', '521006956621e4e7a6a3d16.70681548'); // mettre dans .env

$cinetpaySiteId = env('CINETPAY_SITE_ID', '935132'); // mettre dans .env
...
//    'notify_url' => 'https://sindy-overmeek-congruently.ngrok-free.dev/api/webhooks/cinetpay/notify/deces',
    ...
    // --> AJOUT : return_url et cancel_url

    // Remarque : certains fournisseurs exigent des URLs http(s). Si CinetPay refuse les deep links

    // non-http, utilise le fallbackReturnUrl (page web qui redirige vers le deep link).

    'return_url' => $fallbackReturnUrl, // ou $returnUrl si CinetPay accepte le scheme plateauapps://

    'cancel_url' => $fallbackCancelUrl, // idem
...
return response()->json([
    ...
    'payment_details' => [
        ...
        // on renvoie les urls pour Postman / front
