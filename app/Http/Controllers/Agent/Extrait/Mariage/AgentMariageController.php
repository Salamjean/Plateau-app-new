    ...
    public function updateEtat(Request $request, $id)
    {
        ...
        if ($user && $mariage->etat === 'terminé' && $ancienEtat !== 'terminé') {
            if ($mariage->choix_option === 'livraison') {
                $phoneNumber = $mariage->contact_destinataire;
                $phoneNumber = preg_replace('/[^0-9]/', '', $phoneNumber);
                if (!str_starts_with($phoneNumber, '225') && strlen($phoneNumber) == 10) {
                    $phoneNumber = '225' . $phoneNumber;
                }
                $destName = trim($mariage->prenom_destinataire . ' ' . $mariage->nom_destinataire);
            } else {
                $phoneNumber = $user->indicatif . $user->contact;
                $destName = $user->name;
            }
            $message = "Bonjour {$destName}, votre demande d'extrait de mariage (Réf: {$mariage->reference}) a été traitée.";
            if ($mariage->livraison_code) {
        ...
        if ($user && $mariage->etat === 'rejetée' && $ancienEtat !== 'rejetée') {
            if ($mariage->choix_option === 'livraison') {
                ...
                try {
                    Mail::to($user->email)->send(new DemandeRejeteeMail($user, $mariage, 'mariage', $mariage->motif_de_rejet));
                } catch (\Exception $e) {
                    Log::error("Erreur lors de l'envoi du mail de rejet (mariage) : " . $e->getMessage());
                }
            } else {
                $phoneNumber = $user->indicatif . $user->contact;
                $destName = $user->name;
            }
            $message = "Bonjour {$destName}, votre demande d'extrait de mariage (Réf: {$mariage->reference}) a été rejetée. Veuillez consulter l'application pour plus de détails.";
            try {
                $yellikaSmsService = app(YellikaSmsService::class);
                $yellikaSmsService->sendSms($phoneNumber, $message);
            } catch (\Exception $e) {
