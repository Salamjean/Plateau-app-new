            ...
            if ($groupe->choix_option === 'livraison') {
                ...
                foreach ($groupe->lignes as $ligne) {
                    if (($decisions[$ligne->id] ?? null) === 'rejeter') {
                        $motifsRejet[] = "• Acte #" . $ligne->position_in_groupe . " (" . $ligne->nameDefunt . ") : " . ($motifs[$ligne->id] ?? 'sans précision');
                    }
                }
                if ($motifGlobal) {
                ...
                try {
                    Mail::to($user->email)->send(new DemandeRejeteeMail($user, $groupe, 'deces_groupe', $emailMotif));
                } catch (\Exception $e) {
                    Log::error("Erreur lors de l'envoi du mail de rejet (décès groupe) : " . $e->getMessage());
                }
            } else {
