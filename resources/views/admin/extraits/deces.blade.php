  ...
  <div class="ms-panel-body">
    ...
    <div class="card mb-4" style="border-top: 3px solid #6777ef; border-radius: 8px; box-shadow: 0 4px 6px rgba(0,0,0,0.05);">
        <div class="card-body p-4">
            <form method="GET" action="{{ route('admin.death') }}" class="row g-3 align-items-end">
                <div class="col-md-4">
                ...
                <div class="col-md-3">
                    ...
                    <div class="input-group">
                        ...
                        <input type="text" name="reference" id="reference" class="form-control border-start-0" placeholder="Ex: death-..." value="{{ $searchReference }}" onchange="this.form.submit()">
                    </div>
                </div>
                <div class="col-md-3">
                ...
                <div class="col-md-2 d-grid gap-2">
                    ...
                    <a href="{{ route('admin.death') }}" class="btn btn-secondary w-100 py-2" style="border-radius: 8px; font-weight: 500;">
      ...
      <div class="col-lg-12">
          <div class="card mb-4">
              <div class="card-header py-3">
                  <h6 class="m-0 font-weight-bold text-primary">Liste de toutes les demandes d'extraits de décès</h6>
              </div>
              <div class="table-responsive p-3">
                  ...
                  <table class="table align-items-center table-flush" id="dataTable">
                      ...
                      <tbody>
                          @forelse ($deces as $items)
                              <tr style="font-size: 12px"  class="text-center">
                                  <td>
                                      <div class="fw-bold">{{ strtoupper($items->user->name ?? '').' '.strtoupper($items->user->prenom ?? '') }}</div>
                                      @php
                                          $contactNav = $items->contact_destinataire ?: ($items->user->contact ?? '');
                                      @endphp
                                      @if ($contactNav)
                                  ...
                                  <td>
                                      @php
                                          $qty = (int) ($items->quantite ?? (($items->qty_simple ?? 0) + ($items->qty_integral ?? 0)));
                                      @endphp
