...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .toolbar-title { font-size: 1rem; font-weight: 700; color: var(--text-main); margin: 0; }
        ...
        .toggle-btn.active.pending-tab { color: var(--warning); }
        .toggle-btn.active.treated-tab { color: var(--success); }
        .toggle-count {
            display: inline-flex; align-items: center; justify-content: center;
            min-width: 20px; height: 20px; border-radius: 6px;
            font-size: 0.72rem; font-weight: 700; padding: 0 4px;
        }
        .pending-tab .toggle-count { background: #fef3c7; color: var(--warning); }
        .treated-tab .toggle-count { background: var(--success-light); color: var(--success); }
        ...
        .btn-reset:hover { background: var(--body-bg); color: var(--text-main); }
        ...
        .result-count span { font-weight: 700; color: var(--text-main); }
        ...
        .table-modern th {
            ...
            background: #f8fafc; white-space: nowrap; text-align: center;
        }
        .table-modern td {
            padding: 1.1rem 1.25rem; vertical-align: middle;
        ...
        /* BADGES */
        .badge {
            padding: 0.35em 0.75em; font-size: 0.72rem; font-weight: 700;
            border-radius: 6px; display: inline-flex;
            align-items: center; gap: 0.35rem; letter-spacing: 0.2px;
        }
        .badge-naissance { background: var(--primary-light); color: var(--primary); }
        .badge-deces     { background: var(--purple-light);  color: var(--purple); }
        .badge-mariage   { background: var(--pink-light);    color: var(--pink); }
        .badge-pending   { background: var(--warning-light); color: var(--warning); }
        .badge-done      { background: var(--success-light); color: var(--success); }

        /* DEMANDEUR */
        .demandeur-name    { font-weight: 600; color: var(--text-main); }
        .demandeur-contact { font-size: 0.75rem; color: var(--text-muted); margin-top: 2px; }
        ...
        .qty-badge {
            ...
            background: #f1f5f9; font-weight: 800; font-size: 0.95rem;
            color: var(--text-main); font-family: 'DM Mono', monospace;
        }
        ...
        .btn-done {
            ...
            border: 1px solid #a7f3d0; padding: 0.5rem 1rem;
        ...
        .empty-icon  { font-size: 2.8rem; color: #e2e8f0; margin-bottom: 0.75rem; }
        .empty-state h3 { font-size: 1rem; font-weight: 700; margin: 0 0 0.35rem; color: var(--text-muted); }
    ...
    <div class="stats-grid">
        <div class="stat-card" style="border-left:4px solid var(--warning);">
            ...
            <div>
                ...
                <div class="stat-value">{{ number_format($montantRestant ?? 0, 0, ',', ' ') }} <small>FCFA</small></div>
            </div>
        </div>
        ...
        <div class="stat-card" style="border-left:4px solid var(--primary);">
            ...
            <div>
                ...
                <div class="stat-value">{{ $demandesEnAttente->sum(fn($d) => (int)($d->free_timbres_count ?? 0)) }}</div>
            </div>
        </div>
    </div>
        ...
        <div class="toolbar">
            ...
            <div class="toolbar-row">
                ...
                <div class="toggle-group" role="tablist">
                    <button class="toggle-btn pending-tab active" id="tab-pending" onclick="switchTab('pending')" role="tab">
                        <i class="fas fa-clock"></i>
                        À valider
                        <span class="toggle-count" id="count-pending">{{ $demandesEnAttente->count() }}</span>
                    </button>
                    <button class="toggle-btn treated-tab" id="tab-treated" onclick="switchTab('treated')" role="tab">
            ...
            <table class="table-modern">
                ...
                <tbody id="tableBody">
                    ...
                    <tr class="data-row pending-row"
                        ...
                        <td>
                            <div class="date-main">{{ \Carbon\Carbon::parse($demande->created_at)->locale('fr')->isoFormat('D MMM YYYY') }}</div>
                            <div class="date-time">{{ \Carbon\Carbon::parse($demande->created_at)->format('H:i') }}</div>
                        </td>
                        <td>
                            <span class="qty-badge">{{ $demande->quantite ?? '1' }}</span>
                            @if(($demande->free_timbres_count ?? 0) > 0)
                                <small style="color:var(--success); display:block; font-weight:600; font-size:0.7rem; margin-top:2px;">
                                    ({{ $demande->free_timbres_count ?? 0 }} gratuit{{ ($demande->free_timbres_count ?? 0) > 1 ? 's' : '' }})
                                </small>
                            @endif
                        </td>
                        <td>
                            <span class="badge badge-pending"><i class="fas fa-hourglass-half"></i> En attente</span>
                        </td>
                        <td>
                            <form action="{{ route('comptable.demandes.recover', ['type' => $demande->type_demande, 'id' => $demande->id]) }}"
                    ...
                    <tr class="pending-row empty-pending-row">
                        <td colspan="6">
                            <div class="empty-state">
                                <div class="empty-icon"><i class="fas fa-check-circle" style="color:#a7f3d0;"></i></div>
