...
@section('content')
                    ...
                    <div class="card-body p-4">
                        ...
                        <div class="filters-container mb-4">
                            <div class="filter-row">
                                ...
                                <div class="filter-group">
                                    <label>&nbsp;</label>
                                    <a href="{{ route('comptable.timbre.history') }}" class="btn-primary"
                                            ...
                                            <td>
                                                <div class="d-flex align-items-center"
                                                    ...
                                                    <div class="user-info ms-2">
                                                        {{ $vente->comptable->name ?? 'N/A' }}
                                                        {{ $vente->comptable->prenom ?? '' }}
                                                    </div>
                                                </div>
                                            </td>
                        ...
                        @if($ventes->hasPages())
                            <div class="pagination-container">
                                <div class="pagination-info">
                                    Affichage de {{ $ventes->firstItem() }} à {{ $ventes->lastItem() }} sur
                        ...
                        <div class="export-container">
                            <button class="btn-export daily-pdf" onclick="exporterPDFJournalier()">
                                <i class="fas fa-download me-2"></i> Ventes du jour (PDF)
                            </button>
    ...
    // Fonction pour exporter les ventes du jour en PDF
    function exporterPDFJournalier() {
        try {
            ...
            const headers = [['Date', 'Heure', 'Financier', 'Quantit\u00e9', 'Type']];
            const rows = [];
            const tableRows = document.querySelectorAll('#tableVentes tbody tr.operation-row');
            ...
            tableRows.forEach(row => {
                ...
                if (rowDate === todayISO) {
                    const cells = row.querySelectorAll('td');
                    const date = cells[0].querySelector('.date').textContent.trim();
                    const time = cells[1].querySelector('.time').textContent.trim();
                    const financier = cells[2].textContent.trim();
                    const quantite = cells[3].textContent.trim();
                    const type = cells[4].textContent.trim();
                    rows.push([date, time, financier, quantite, type]);
            ...
            if (rows.length === 0) {
                ...
                doc.text('Aucune vente enregistr\u00e9e pour aujourd\'hui.', 105, 70, { align: 'center' });
            } else {
                doc.autoTable({
                    head: headers,
                    body: rows,
            ...
            showToast('Succ\u00e8s', 'PDF des ventes du jour t\u00e9l\u00e9charg\u00e9 !', 'success');
        } catch (error) {
            console.error('Erreur PDF journalier:', error);
            showToast('Erreur', 'Erreur lors de la g\u00e9n\u00e9ration du PDF', 'error');
        }
    }
    </script>
