                ...
                <div class="card-header py-3 bg-white">
                    <div class="row align-items-center">
                        <div class="col text-center">
                            <h2 class="mb-0 fw-bold text-primary"><i class="fas fa-users me-2"></i>Gestion du service financier</h2>
                            <p class="text-muted mb-0">Informations sur le service financier</p>
                    ...
                    <div class="d-flex justify-content-between align-items-center mt-4">
                        ...
                        <nav aria-label="Page navigation">
                            <ul class="pagination rounded-pill">
                                <li class="page-item {{ $finances->onFirstPage() ? 'disabled' : '' }}">
                                    <a class="page-link rounded-start-pill" href="{{ $finances->previousPageUrl() }}">
                                        <i class="fas fa-chevron-left"></i>
                                    </a>
                                </li>
                                
                                @for ($i = 1; $i <= $finances->lastPage(); $i++)
                                <li class="page-item {{ $finances->currentPage() == $i ? 'active' : '' }}">
                                    <a class="page-link" href="{{ $finances->url($i) }}">{{ $i }}</a>
                                </li>
                                @endfor
                                
                                <li class="page-item {{ $finances->hasMorePages() ? '' : 'disabled' }}">
                                    <a class="page-link rounded-end-pill" href="{{ $finances->nextPageUrl() }}">
