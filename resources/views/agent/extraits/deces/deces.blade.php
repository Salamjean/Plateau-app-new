...
.pagination .page-link:hover:not(.disabled) {
  ...
  <div class="dashboard-container">
    ...
    <div style="display: flex; gap: 1rem; flex-wrap: wrap; margin-bottom: 1.5rem;">
      ...
      <a href="{{ route('agent.demandes.deces.groupes.index') }}"
         style="flex: 1; min-width: 250px; background: linear-gradient(135deg, #EE5D50 0%, #f97c70 100%); color: #fff; padding: 1.5rem; border-radius: 16px; text-decoration: none; display: flex; align-items: center; gap: 16px; box-shadow: 0 4px 15px rgba(238,93,80,0.2);">
                ...
                <tr>
                  ...
                  <td style="text-align: center">
                    @if($dece->CNIdfnt)
                      ...
                      @else
                        <img src="{{ $CNIPath }}" alt="CNI" style="width: 40px; height:40px" class="document-preview"
                    ...
                    @if($dece->CNIdcl)
                      ...
                      @else
                        <img src="{{ $CNIdclPath }}" alt="CNIdcl" style="width: 40px; height:40px" class="document-preview"
                          data-bs-toggle="modal" data-bs-target="#imageModal" onclick="showImage(this)"
                          onerror="this.onerror=null; this.src='{{ asset('assets/images/profiles/bébé.jpg') }}'">
                      @endif
                    @else
                    ...
                    @if (!empty($dece->documentMariage))
                      ...
                      @else
                        <img src="{{ $documentMariagePath }}" alt="documentMariage" style="width: 40px; height:40px"
                          class="document-preview" data-bs-toggle="modal" data-bs-target="#imageModal" onclick="showImage(this)"
                          onerror="this.onerror=null; this.src='{{ asset('assets/images/profiles/bébé.jpg') }}'">
                      @endif
                    @endif
                    @if (!empty($dece->RequisPolice))
                      ...
                      @else
                        <img src="{{ $RequisPolicePath }}" alt="RequisPolice" style="width: 40px; height:40px"
                          class="document-preview" data-bs-toggle="modal" data-bs-target="#imageModal" onclick="showImage(this)"
                          onerror="this.onerror=null; this.src='{{ asset('assets/images/profiles/bébé.jpg') }}'">
                      @endif
                    @endif
                  ...
                  <td style="text-align: center" data-label="Actions">
                    ...
                    <button class="btn-action btn-icon" style="background-color: #17a2b8;"
                      onclick="showRequestDetails({{ json_encode($dece) }})" title="Voir les détails de la demande">
                    ...
                    @else
                      <a href="{{ route('agent.demandes.deces.edit', $dece->id) }}" class="btn-action btn-secondary btn-icon"
                  ...
                  <td style="text-align: center">
                    <div class="d-flex justify-content-center gap-2">
                      @if($dece->choix_option == 'livraison')
                        <a href="#" class="delivery-badge badge" style="background-color: #2c7873;" data-bs-toggle="modal"
                          data-bs-target="#livraisonModal" onclick="showLivraisonModal({{ json_encode($dece) }})">
                        ...
                        @if($dece->date_livraison)
                          <div style="font-size: 0.7rem; color: #666; margin-top: 5px;">
                            ...
                            @if($dece->heure_livraison)
                              <br><i class="fas fa-clock"></i> {{ \Carbon\Carbon::parse($dece->heure_livraison)->format('H:i') }}
                            @endif
                          </div>
                        @endif
                      @else
        ...
        @if($deces->hasPages())
          <div class="d-flex justify-content-between align-items-center mt-4">
            <div class="text-muted">
              Affichage de <strong>{{ $deces->firstItem() }}</strong> à <strong>{{ $deces->lastItem() }}</strong> sur
