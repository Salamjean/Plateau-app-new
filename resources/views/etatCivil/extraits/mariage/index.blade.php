...
@section('content')
    ...
    <div class="dashboard-container">
        <div class="page-title">
            <h2><i class="fas fa-ring me-2"></i>Demandes d'extraits de mariage</h2>
            ...
            <form method="GET" action="{{ route('etat_civil.request.wedding') }}" class="filter-form">
                ...
                <div class="d-flex gap-2">
                    ...
                    @if (request()->anyFilled(['etat', 'type', 'livraison']))
                        <a href="{{ route('etat_civil.request.wedding') }}" class="btn-action"
                ...
                <table class="table">
                    ...
                    <tbody>
                        @forelse($mariages as $mariage)
                            <tr>
                                <td data-label="Référence"><span class="id-badge">{{ $mariage->reference }}</span></td>
                                <td data-label="Demandeur">
                                    <div class="user-info-cell">
                                        <div class="user-avatar-small">{{ substr($mariage->user->name ?? 'U', 0, 1) }}
                                        </div>
                                        <span class="user-name">{{ $mariage->user->name ?? 'N/A' }}</span>
                                    </div>
                                </td>
                                <td data-label="Type">
                                    <span class="fw-600">{{ $mariage->type == 'simple' ? 'Simple' : 'Intégrale' }}</span>
                                </td>
                                <td data-label="Quantité">{{ $mariage->quantite }} copie(s)</td>
                                <td data-label="Date">{{ $mariage->created_at->format('d/m/Y') }}</td>
                                <td data-label="État">
                                    <span class="status-badge status-{{ str_replace(' ', '-', $mariage->etat) }}">
                                        {{ ucfirst($mariage->etat) }}
                                    </span>
                                </td>
                                ...
                                <td data-label="Agent">
                                    <span class="text-muted">{{ $mariage->agent->name ?? '---' }}</span>
                                </td>
                            </tr>
                        @empty
                            <tr>
