...
<style>
    ...
    .step-item.active .step-number { background: var(--danger, #EE5D50); color: #fff; }
    ...
    .step-item.active .step-label { color: var(--danger, #EE5D50); }
    .form-step { display: none; animation: fadeIn 0.4s ease-out; }
    .form-step.active { display: block; }
    .form-section { margin-bottom: 2rem; }
    .form-section-title { display: flex; align-items: center; margin-bottom: 1.25rem; color: var(--danger, #EE5D50); font-weight: 700; font-size: 1.05rem; text-transform: uppercase; letter-spacing: 1px; }
    .form-section-title i { width: 35px; height: 35px; background: rgba(238,93,80,0.1); display: flex; align-items: center; justify-content: center; border-radius: 10px; margin-right: 12px; font-size: 14px; }
    ...
    .form-control-custom:focus { border-color: var(--danger, #EE5D50); background: #fff; outline: none; }
    ...
    .quantity-card.has-value { border-color: var(--danger, #EE5D50); background: #fef5f4; }
    ...
    .qty-btn { width: 38px; height: 38px; border-radius: 12px; border: none; background: #f4f7fe; color: var(--danger, #EE5D50); font-weight: 800; font-size: 18px; cursor: pointer; transition: 0.2s; }
    .qty-btn:hover { background: var(--danger, #EE5D50); color: #fff; }
    ...
    .total-recap { background: linear-gradient(135deg, var(--danger, #EE5D50) 0%, #f97c70 100%); color: #fff; padding: 1.5rem 2rem; border-radius: 20px; margin-top: 1.5rem; box-shadow: 0 10px 30px rgba(238,93,80,0.25); }
    ...
    .ligne-card.integral { border-left: 5px solid var(--danger, #EE5D50); }
    ...
    .ligne-badge.integral { background: rgba(238,93,80,0.1); color: var(--danger, #EE5D50); }
    ...
    .file-upload-area:hover { border-color: var(--danger, #EE5D50); background: #fef5f4; }
    .file-upload-area i { font-size: 1.8rem; color: var(--danger, #EE5D50); margin-bottom: 0.5rem; }
    ...
    .delivery-option-content i { font-size: 2rem; margin-bottom: 1rem; color: var(--danger, #EE5D50); }
    .delivery-option-card input:checked + .delivery-option-content { border-color: var(--danger, #EE5D50); background: #fef5f4; box-shadow: 0 8px 25px rgba(238,93,80,0.15); }
    ...
    .btn-next { background: var(--danger, #EE5D50); color: #fff; border: none; }
    .btn-next:hover { background: #c34a3f; transform: translateX(3px); }
    ...
    .payment-option input:checked + .payment-option-content { border-color: var(--danger, #EE5D50); background: #fef5f4; }
    /* Radio cards (Célibataire/Marié, Milieu médical/Hors) */
    .radio-card-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 0.75rem; }
    .radio-card { position: relative; cursor: pointer; }
    .radio-card input { position: absolute; opacity: 0; }
    .radio-card-content {
        padding: 14px 16px; background: #fff; border: 2px solid #f4f7fe;
        border-radius: 14px; text-align: center; font-weight: 700;
        color: var(--text-navy); transition: 0.2s; font-size: 0.9rem;
    }
    .radio-card input:checked + .radio-card-content {
        border-color: var(--danger, #EE5D50); background: #fef5f4;
...
<div class="form-page-container">
    <div class="form-glass-card">
        <div class="form-header-box">
            <h2><i class="fas fa-cross" style="color: var(--danger, #EE5D50); margin-right: 10px;"></i> Demande groupée — Décès</h2>
            <p>Commandez plusieurs actes de décès en une seule démarche</p>
        ...
        <form id="decesGroupeeForm" action="{{ route('user.extrait.deces.groupee.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-step active" id="step-1">
                <div class="form-section">
                    ...
                    <div class="quantity-card" id="card-simple">
                        ...
                        <div style="color:#718096; font-size:0.85rem;">{{ \App\Http\Controllers\User\Extrait\Deces\DecesGroupeController::TARIF_TIMBRE }} FCFA / copie</div>
                    </div>
                    <div class="quantity-card" id="card-integral">
                        <div class="quantity-card-header">
                            <div class="quantity-card-title"><i class="fas fa-file-contract" style="background: rgba(238,93,80,0.1); color: var(--danger, #EE5D50);"></i> Copie intégrale</div>
                            <div class="qty-stepper">
                ...
                <div class="stepper-footer">
                    <a href="{{ route('user.extrait.deces.create') }}" class="btn-step btn-prev" style="text-decoration: none;"><i class="fas fa-arrow-left"></i> Annuler</a>
            ...
            <div class="form-step" id="step-2">
                <div class="form-section">
                    ...
                    <p style="color:#718096; margin-bottom:1.5rem;">Renseignez les informations pour chaque défunt concerné.</p>
            ...
            <div class="form-step" id="step-3">
                <div class="form-section">
                    ...
                    <div class="delivery-card-grid">
                        ...
                        <label class="delivery-option-card">
                            ...
                            <div class="delivery-option-content"><i class="fas fa-motorcycle"></i><h5>Livraison Express</h5><p>{{ \App\Http\Controllers\User\Extrait\Deces\DecesGroupeController::TARIF_LIVRAISON }} FCFA - Domicile</p></div>
                        </label>
                    </div>
                </div>
                ...
                <div class="form-section" id="payment-method-section" style="display:none;">
                    ...
                    <div class="payment-grid">
                        ...
                        <label class="payment-option"><input type="radio" name="payment_method" value="mtn" onchange="onPaymentMethodChange()"><div class="payment-option-content"><img src="{{ asset('assets/assets/img/mtn.png') }}" alt="MTN" onerror="this.style.display='none'"><h6>MTN Money</h6><small>Mobile Money</small></div></label>
                ...
                <div class="form-section">
                    ...
                    <div class="total-recap">
                        ...
                        <div class="total-recap-row" id="final-livraison-row" style="display:none;"><span><i class="fas fa-motorcycle"></i> Livraison</span><span>{{ \App\Http\Controllers\User\Extrait\Deces\DecesGroupeController::TARIF_LIVRAISON }} FCFA</span></div>
...
<script>
    const TARIF_TIMBRE = {{ \App\Http\Controllers\User\Extrait\Deces\DecesGroupeController::TARIF_TIMBRE }};
    const TARIF_LIVRAISON = {{ \App\Http\Controllers\User\Extrait\Deces\DecesGroupeController::TARIF_LIVRAISON }};
    ...
    function buildLigne(position, type) {
        ...
        card.innerHTML = `
            <div class="ligne-card-header">
                <div class="ligne-card-title"><i class="fas ${isIntegral ? 'fa-file-contract' : 'fa-file-alt'}"></i> Décès n°${position + 1}</div>
            ...
            <div class="form-section-title" style="font-size: 0.95rem;">
                <i class="fas fa-user"></i> Identité du défunt
            </div>
            <div class="row">
                <div class="col-md-12 mb-3">
                    <div class="input-group-custom">
                        ...
                        <div class="input-wrapper">
                            <input type="text" name="lignes[${position}][name]" class="form-control-custom" placeholder="Nom complet" required>
            ...
            <div class="row">
                <div class="col-md-4 mb-3">
                    <div class="input-group-custom">
                        ...
                        <div class="input-wrapper">
                            <input type="text" name="lignes[${position}][numberR]" class="form-control-custom" placeholder="Ex: 1234/2024">
                ...
                <div class="col-md-4 mb-3">
                    <div class="input-group-custom">
                        ...
                        <div class="input-wrapper">
                            <input type="date" name="lignes[${position}][dateR]" class="form-control-custom">
                ...
                <div class="col-md-4 mb-3">
                    <div class="input-group-custom">
                        ...
                        <div class="input-wrapper">
                            <input type="text" name="lignes[${position}][commune_deces]" class="form-control-custom" placeholder="Ville/Commune" required>
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    <div class="input-group-custom">
                        ...
                        <div class="input-wrapper">
                            <input type="text" name="lignes[${position}][nom_prenoms_pere]" class="form-control-custom" placeholder="Nom complet du père">
                ...
                <div class="col-md-6 mb-3">
                    <div class="input-group-custom">
                        ...
                        <div class="input-wrapper">
                            <input type="text" name="lignes[${position}][nom_prenoms_mere]" class="form-control-custom" placeholder="Nom complet de la mère">
                ...
                <div class="col-md-6 mb-3">
                    <div class="input-group-custom">
                        ...
                        <label class="file-upload-area">
                            ...
                            <h6 id="file-name-${position}-dfnt" style="margin: 5px 0; color: var(--text-navy); font-weight: 700;">Téléverser</h6>
                            <p style="font-size: 0.8rem; color: #718096; margin: 0;">PDF, JPG ou PNG (max 5 Mo)</p>
                            <input type="file" name="lignes[${position}][CNIdfnt]" class="d-none" required accept=".jpg,.jpeg,.png,.pdf"
                                   onchange="document.getElementById('file-name-${position}-dfnt').textContent = this.files[0]?.name || 'Téléverser'">
                        </label>
                    </div>
                </div>
                <div class="col-md-6 mb-3">
                    <div class="input-group-custom">
                        ...
                        <label class="file-upload-area">
                            <i class="fas fa-cloud-upload-alt"></i>
                            <h6 id="file-name-${position}-dcl" style="margin: 5px 0; color: var(--text-navy); font-weight: 700;">Téléverser</h6>
                            <p style="font-size: 0.8rem; color: #718096; margin: 0;">PDF, JPG ou PNG (max 5 Mo)</p>
                            <input type="file" name="lignes[${position}][CNIdcl]" class="d-none" required accept=".jpg,.jpeg,.png,.pdf"
                                   onchange="document.getElementById('file-name-${position}-dcl').textContent = this.files[0]?.name || 'Téléverser'">
                        </label>
                    </div>
                </div>
            </div>
            ...
            <div class="row" style="margin-top: 1.5rem;">
                <div class="col-md-6 mb-3">
                    ...
                    <div class="radio-card-grid">
                        <label class="radio-card">
                            <input type="radio" name="lignes[${position}][married]" value="non" checked
                                   onchange="toggleDocumentMariage(${position}, false)">
                            <div class="radio-card-content">Célibataire</div>
                        </label>
                        <label class="radio-card">
                            <input type="radio" name="lignes[${position}][married]" value="oui"
                                   onchange="toggleDocumentMariage(${position}, true)">
                            <div class="radio-card-content">Marié(e)</div>
                        </label>
                    </div>
                    <div class="input-group-custom" id="document-mariage-${position}" style="display:none; margin-top: 1rem;">
                        <label>Acte de mariage du défunt : *</label>
                        <label class="file-upload-area">
                            <i class="fas fa-cloud-upload-alt"></i>
                            <h6 id="file-name-${position}-mariage" style="margin: 5px 0; color: var(--text-navy); font-weight: 700;">Téléverser</h6>
                            <p style="font-size: 0.8rem; color: #718096; margin: 0;">PDF, JPG ou PNG</p>
                            <input type="file" name="lignes[${position}][documentMariage]" class="d-none" accept=".jpg,.jpeg,.png,.pdf"
                                   onchange="document.getElementById('file-name-${position}-mariage').textContent = this.files[0]?.name || 'Téléverser'">
                        </label>
                    </div>
                </div>
                ...
                <div class="col-md-6 mb-3">
                    <div class="form-section-title" style="font-size: 0.95rem;">
                        <i class="fas fa-hospital"></i> Lieu du décès
                    </div>
                    <div class="radio-card-grid">
                        <label class="radio-card">
                            <input type="radio" name="lignes[${position}][DecesHorsS]" value="non" checked
                                   onchange="toggleRequisPolice(${position}, false)">
                            <div class="radio-card-content">En milieu médical</div>
                        </label>
                        <label class="radio-card">
                            <input type="radio" name="lignes[${position}][DecesHorsS]" value="oui"
                                   onchange="toggleRequisPolice(${position}, true)">
                            <div class="radio-card-content">Hors milieu médical</div>
                        </label>
                    </div>
                    <div class="input-group-custom" id="requis-police-${position}" style="display:none; margin-top: 1rem;">
                        <label>Réquisition de police : *</label>
                        <label class="file-upload-area">
                            <i class="fas fa-cloud-upload-alt"></i>
                            <h6 id="file-name-${position}-police" style="margin: 5px 0; color: var(--text-navy); font-weight: 700;">Téléverser</h6>
                            <p style="font-size: 0.8rem; color: #718096; margin: 0;">PDF, JPG ou PNG</p>
                            <input type="file" name="lignes[${position}][RequisPolice]" class="d-none" accept=".jpg,.jpeg,.png,.pdf"
                                   onchange="document.getElementById('file-name-${position}-police').textContent = this.files[0]?.name || 'Téléverser'">
                        </label>
                    </div>
                </div>
            </div>
    ...
    // Toggle document mariage si marié
    function toggleDocumentMariage(position, isMarried) {
        const block = document.getElementById('document-mariage-' + position);
    ...
    // Toggle réquisition police si décès hors milieu médical
    function toggleRequisPolice(position, isHorsMilieu) {
        const block = document.getElementById('requis-police-' + position);
        const input = block.querySelector('input[type="file"]');
        if (isHorsMilieu) {
            block.style.display = 'block';
            input.setAttribute('required', 'required');
    ...
    document.addEventListener('DOMContentLoaded', function() {
        const form = document.getElementById('decesGroupeeForm');
        if (!form) return;
        form.addEventListener('submit', async function(e) {
            ...
            try {
                ...
                if (data.payment_url && paymentTab) {
                    ...
                    window.location.href = data.redirect_url || '{{ route("user.extrait.deces.index") }}';
                } else {
                    if (paymentTab) paymentTab.close();
                    window.location.href = data.redirect_url || '{{ route("user.extrait.deces.index") }}';
                }
