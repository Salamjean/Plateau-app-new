                                ...
                                <td class="text-center">
                                    <div class="d-flex gap-2 justify-content-center">
                                        ...
                                        @elseif (is_null($item->agent_id))
                                            <a href="{{ route('user.extrait.deces.edit', $item->id) }}"
                                                class="btn-action btn-edit" title="Modifier">
                                                <i class="fas fa-edit"></i>
                                            </a>
                                        @endif
                        ...
                        .then(data => {
                            if (data.success) {
                                if (data.redirect_url && data.mtn_ref && data.reference) {
                                    ...
                                    startMtnPaymentPolling(data.reference, data.mtn_ref, 'deces');
                ...
                .then(data => {
                    if (data.status === 'SUCCESSFUL') {
                        Swal.fire({
                            ...
                            window.location.href = data.redirect || "{{ route('user.extrait.deces.index') }}";
                        });
